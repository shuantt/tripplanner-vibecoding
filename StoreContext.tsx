...
const reducer = (state: AppState, action: Action): AppState => {
  switch (action.type) {
    ...
    case 'SWAP_DAYS': {
      ...
      return { ...state, itinerary: state.itinerary.map(item => {
    ...
    case 'REORDER_REC': {
        ...
        return { ...state, recommendations: state.recommendations.map(r => {
...
export const StoreProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const importTripById = async (shortId: string): Promise<boolean> => {
      setIsLoading(true);
      try {
          const res = await fetch(`/api/trip/${shortId.toUpperCase()}`);
