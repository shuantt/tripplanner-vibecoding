...
import { Plus, Trash2, Calendar, DollarSign, Heart, FileText, ChevronLeft, Briefcase, Users, Calendar as CalendarIcon, Settings as SettingsIcon, Copy, Share2, Link } from 'lucide-react';
  ...
  if (!activeTrip) {
    return (
      <div className="h-[100dvh] bg-gray-50 p-6 flex flex-col overflow-y-auto relative">
        ...
        <header className="mb-8 mt-4 shrink-0">
          <h1 className="text-3xl font-extrabold text-gray-900">我的旅程</h1>
          <p className="text-gray-500 mt-2">建立您的第一個旅遊筆記本</p>
        </header>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pb-20 overflow-y-auto">
          {state.trips.map(trip => (
            <TripCard key={trip.id} trip={trip} onClick={() => setActiveTripId(trip.id)} onDelete={(e) => { e.stopPropagation(); if(confirm('確定要刪除嗎？')) dispatch({type:'DELETE_TRIP', payload:trip.id})}} onEdit={(e) => {
...
const App = () => <StoreProvider><MainApp /></StoreProvider>;
export default App;